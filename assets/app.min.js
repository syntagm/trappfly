...
;var google = google || {};
google.bookmarkbubble = google.bookmarkbubble || {}, google.bind = function(e, o) {
    return function() {
    ...
    try {
        var e = this.LOCAL_STORAGE_PREFIX + this.DISMISSED_,
            o = Number(window.localStorage[e]) || 0;
        return o >= this.NUMBER_OF_TIMES_TO_DISMISS
    } catch (t) {
        return !0
    }
...
}, google.bookmarkbubble.Bubble.prototype.getVersion_ = function(e, o, t, i) {
    return e << 21 | o << 14 | t << 7 | i
}, google.bookmarkbubble.Bubble.prototype.getIosVersion_ = function() {
    var e = this.IOS_VERSION_USERAGENT_REGEX_.exec(window.navigator.userAgent) || [];
    return e.shift(), this.getVersion_.apply(this, e)
}, google.bookmarkbubble.Bubble.prototype.getAndroidVersion_ = function() {
    var e = this.ANDROID_VERSION_USERAGENT_REGEX_.exec(window.navigator.userAgent) || [];
    return e.shift(), this.getVersion_.apply(this, e)
    ...
    this.hasBeenDestroyed_ || (this.element_.style.WebkitTransition = "-webkit-transform 0.7s ease-in", this.element_.style.WebkitTransform = "translate3d(0," + this.getHiddenYPosition_() + "px,0)", window.setTimeout(google.bind(this.destroy, this), 700))
}, google.bookmarkbubble.Bubble.prototype.getVisibleYPosition_ = function() {
    return this.isIpad_() || this.isPlayBook_() || this.getAndroidVersion_() >= this.getVersion_(3, 0) ? window.pageYOffset + 17 : window.pageYOffset - this.element_.offsetHeight + window.innerHeight - 17
}, google.bookmarkbubble.Bubble.prototype.getHiddenYPosition_ = function() {
    return this.isIpad_() || this.isPlayBook_() || this.getAndroidVersion_() >= this.getVersion_(3, 0) ? window.pageYOffset - this.element_.offsetHeight : window.pageYOffset + window.innerHeight
}, google.bookmarkbubble.Bubble.prototype.iconUrl_, google.bookmarkbubble.Bubble.prototype.getIconUrl_ = function() {
    if (!this.iconUrl_) {
        var e = this.getLink(this.REL_ICON_);
        e && (this.iconUrl_ = e.href) || (this.iconUrl_ = "data:image/png;base64,")
    }
    return this.iconUrl_
}, google.bookmarkbubble.Bubble.prototype.getLink = function(e) {
    e = e.toLowerCase();
    for (var o = document.getElementsByTagName("link"), t = 0; t < o.length; ++t) {
        var i = o[t];
        if (i.getAttribute("rel").toLowerCase() == e) return i
    }
    return null
    ...
    var e = document.createElement("div"),
        ...
        t = this.isAndroid_(),
        i = this.isPlayBook_(),
        b = this.isBlackBerry_();
    e.style.position = "absolute", e.style.zIndex = 1e5, e.style.width = "100%", e.style.left = "0", e.style.top = "0";
    var l = document.createElement("div");
    l.style.position = "relative", l.style.width = "180px", l.style.margin = this.getAndroidVersion_() >= this.getVersion_(3, 0) ? "0 0 0 " + (window.innerWidth - 206) + "px" : o ? "0 0 0 82px" : "0 auto", l.style.border = "2px solid #fff", l.style.padding = "1em 1.5em 1em 0.5em", l.style.WebkitBorderRadius = "8px", l.style.WebkitBoxShadow = "0 0 8px rgba(0, 0, 0, 0.7)", l.style.WebkitBackgroundSize = "100% 8px", l.style.backgroundColor = "#eee", l.style.background = "from(#cddcf3), to(#b3caed)) no-repeat top", l.style.font = "0.75em sans-serif", e.appendChild(l), t ? (l.style.font = "0.625em sans-serif", l.innerHTML = this.getAndroidVersion_() < this.getVersion_(3, 0) ? this.msg.android : this.getAndroidVersion_() < this.getVersion_(4, 0) && this.isMobile_() ? this.msg.android3 : this.getAndroidVersion_() < this.getVersion_(4, 1) && this.isMobile_() ? this.msg.android4 : this.getAndroidVersion_() < this.getVersion_(4, 4) && this.isMobile_() ? this.msg.android41 : this.msg.android44) : b ? l.innerHTML = this.msg.blackberry : i ? (l.innerHTML = this.msg.playbook, l.style.position = "absolute", l.style.right = "0px") : l.innerHTML = this.getIosVersion_() >= this.getVersion_(7, 0) ? this.msg.ios7orlater : this.getIosVersion_() >= this.getVersion_(4, 2) ? this.msg.ios42orlater : this.msg.ioslegacy;
    var A = document.createElement("div");
    A.style["float"] = "left", A.style.width = "55px", A.style.height = "55px", A.style.margin = "-2px 17px 3px 5px", A.style.background = "#fff url(" + this.getIconUrl_() + ") no-repeat 0 0", A.style.WebkitBackgroundSize = "contain", A.style.WebkitBorderRadius = "10px", A.style.WebkitBoxShadow = "0 2px 5px rgba(0, 0, 0, 0.4)";
    var n = document.createElement("div");
    n.style.backgroundImage = "url(" + this.IMAGE_ARROW_DATA_URL_ + ")", n.style.width = "25px", n.style.height = "19px", n.style.position = "absolute", n.style.left = "10px", o || i ? (n.style.WebkitTransform = "rotate(180deg)", n.style.top = "-119px", n.style.left = "10px") : this.getAndroidVersion_() >= this.getVersion_(3, 0) ? (n.style.WebkitTransform = "scale(1, -1)", n.style.top = "-19px", n.style.left = "156px") : (n.style.bottom = "-19px", n.style.left = "80px"), l.appendChild(n);
    var r = document.createElement("a");
    return r.onclick = google.bind(this.closeClickHandler_, this), r.style.position = "absolute", r.style.display = "block", r.style.top = "-5px", r.style.right = "-5px", r.style.width = "24px", r.style.height = "24px", r.style.border = "10px solid transparent", r.style.background = "url(" + this.IMAGE_CLOSE_DATA_URL_ + ") no-repeat", r.style.WebkitBackgroundSize = "contain", l.appendChild(r), e
}, "undefined" == typeof page_popup_bubble && (page_popup_bubble = "#home"), $(document).bind("mobileinit", function() {
    $(document).on("pageinit", function() {
        window.setTimeout(function() {
            var e = new google.bookmarkbubble.Bubble,
                o = page_popup_bubble;
            e.hasHashParameter = function() {
                return "" == location.hash && location.href.indexOf(o) == location.href.length - 1
            }, e.setHashParameter = function() {
                this.hasHashParameter() || (location.href = o)
            }, e.getViewportHeight = function() {
                return window.console.log("Example of how to override getViewportHeight."), window.innerHeight
            }, e.getViewportScrollY = function() {
                return window.console.log("Example of how to override getViewportScrollY."), window.pageYOffset
            }, e.registerScrollHandler = function(e) {
                window.console.log("Example of how to override registerScrollHandler."), window.addEventListener("scroll", e, !1)
            }, e.deregisterScrollHandler = function(e) {
                window.console.log("Example of how to override deregisterScrollHandler."), window.removeEventListener("scroll", e, !1)
            }, e.showIfAllowed()
        }, 1e3)
    })
});
...
;window.Modernizr=function(a,b,c){function d(a){o.cssText=a}function e(a,b){return d(prefixes.join(a+";")+(b||""))}function f(a,b){return typeof a===b}function g(a,b){return!!~(""+a).indexOf(b)}function h(a,b,d){for(var e in a){var g=b[a[e]];if(g!==c)return d===!1?a[e]:f(g,"function")?g.bind(d||b):g}return!1}function i(){k.inputtypes=function(a){for(var d=0,e,f,g,h=a.length;d<h;d++)p.setAttribute("type",f=a[d]),e=p.type!=="text",e&&(p.value=q,p.style.cssText="position:absolute;visibility:hidden;",/^range$/.test(f)&&p.style.WebkitAppearance!==c?(l.appendChild(p),g=b.defaultView,e=g.getComputedStyle&&g.getComputedStyle(p,null).WebkitAppearance!=="textfield"&&p.offsetHeight!==0,l.removeChild(p)):/^(search|tel)$/.test(f)||(/^(url|email)$/.test(f)?e=p.checkValidity&&p.checkValidity()===!1:e=p.value!=q)),t[a[d]]=!!e;return t}("search tel url email datetime date month week time datetime-local number range color".split(" "))}var j="2.6.2",k={},l=b.documentElement,m="modernizr",n=b.createElement(m),o=n.style,p=b.createElement("input"),q=":)",r={}.toString,s={},t={},u={},v=[],w=v.slice,x,y={}.hasOwnProperty,z;!f(y,"undefined")&&!f(y.call,"undefined")?z=function(a,b){return y.call(a,b)}:z=function(a,b){return b in a&&f(a.constructor.prototype[b],"undefined")},Function.prototype.bind||(Function.prototype.bind=function(a){var b=this;if(typeof b!="function")throw new TypeError;var c=w.call(arguments,1),d=function(){if(this instanceof d){var e=function(){};e.prototype=b.prototype;var f=new e,g=b.apply(f,c.concat(w.call(arguments)));return Object(g)===g?g:f}return b.apply(a,c.concat(w.call(arguments)))};return d});for(var A in s)z(s,A)&&(x=A.toLowerCase(),k[x]=s[A](),v.push((k[x]?"":"no-")+x));return k.input||i(),k.addTest=function(a,b){if(typeof a=="object")for(var d in a)z(a,d)&&k.addTest(d,a[d]);else{a=a.toLowerCase();if(k[a]!==c)return k;b=typeof b=="function"?b():b,typeof enableClasses!="undefined"&&enableClasses&&(l.className+=" "+(b?"":"no-")+a),k[a]=b}return k},d(""),n=p=null,k._version=j,k}(this,this.document)
;;(function(a){function k(b,c){function j(){var a=document.body,b=document.documentElement;return Math.max(a.scrollHeight,a.offsetHeight,b.clientHeight,b.scrollHeight,b.offsetHeight)}function u(b){e=a("li.dw-v",b).eq(0).index(),d=a("li.dw-v",b).eq(-1).index(),f=i.height,F=m}function x(b){var a=i.headerText;return a?"function"==typeof a?a.call(B,b):a.replace(/{value}/i,b):""}function p(){m.temp=J&&null!==m.val&&m.val!=b.val()||null===m.values?i.parseValue(b.val()?b.val():"",m):m.values.slice(0),m.setValue(!0)}function z(b,c,e,u,f){i.validate.call(B,y,e),a(".dww ul",y).each(function(u){var i=a(this),d=a('li[data-val="'+m.temp[u]+'"]',i),i=d.index(),g=d,d=i;if(!g.hasClass("dw-v")){for(var h=g,x=0,j=0;h.prev().length&&!h.hasClass("dw-v");)h=h.prev(),x++;for(;g.next().length&&!g.hasClass("dw-v");)g=g.next(),j++;j<x&&j&&1==!f||!x||!h.hasClass("dw-v")||1==f?d+=j:(g=h,d-=x),m.temp[u]=g.data("val")}(i!=d||u==e||void 0===e)&&m.scroll(a(this),d,u==e?b:0,c,u)}),m.change(u)}function k(){var b=0,c=0,u=a(window).width(),e=a(window).height(),d=a(window).scrollTop(),i=a(".dwo",y),f=a(".dw",y),g,h;a(".dwc",y).each(function(){g=a(this).outerWidth(!0),b+=g,c=g>c?g:c}),g=b>u?c:b,f.width(g),g=f.outerWidth(),h=f.outerHeight(),f.css({left:(u-g)/2,top:d+(e-h)/2}),i.height(0).height(j())}function w(b){var a=+b.data("pos")+1;l(b,a>d?e:a,1)}function E(b){var a=+b.data("pos")-1;l(b,a<e?d:a,2)}var m=this,B=b,b=a(B),M,i=a.extend({},D),O,y,N={},P={},J=b.is("input"),K=!1;m.enable=function(){i.disabled=!1,J&&b.prop("disabled",!1)},m.disable=function(){i.disabled=!0,J&&b.prop("disabled",!0)},m.scroll=function(b,a,c,u,e){var d=(O-a)*i.height;b.attr("style",(c?H+"-transition:all "+c.toFixed(1)+"s ease-out;":"")+(I?H+"-transform:translate3d(0,"+d+"px,0);":"top:"+d+"px;"));if(c){var g=0;clearInterval(N[e]),N[e]=setInterval(function(){g+=.1,b.data("pos",Math.round((a-u)*Math.sin(g/c*(Math.PI/2))+u)),g>=c&&(clearInterval(N[e]),b.data("pos",a))},100),clearTimeout(P[e]),P[e]=setTimeout(function(){"mixed"==i.mode&&!b.hasClass("dwa")&&b.closest(".dwwl").find(".dwwb").fadeIn("fast")},1e3*c)}else b.data("pos",a)},m.setValue=function(a,c,e){var u=i.formatResult(m.temp);m.val=u,m.values=m.temp.slice(0),K&&a&&z(e),c&&J&&b.val(u).trigger("change")},m.validate=function(b,a,c,u){z(b,a,c,!0,u)},m.change=function(b){var c=i.formatResult(m.temp);"inline"==i.display?m.setValue(!1,b):a(".dwv",y).html(x(c)),b&&i.onChange.call(B,c,m)},m.hide=function(){if(!1===i.onClose.call(B,m.val,m))return!1;a(".dwtd").prop("disabled",!1).removeClass("dwtd"),b.blur(),y&&y.remove(),K=!1,a(window).unbind(".dw")},m.show=function(){if(i.disabled||K)return!1;var c=i.height,e=i.rows*c;p();for(var d='<div class="'+i.theme+'">'+("inline"==i.display?'<div class="dw dwbg dwi"><div class="dwwr">':'<div class="dwo"></div><div class="dw dwbg"><div class="dwwr">'+(i.headerText?'<div class="dwv"></div>':"")),f=0;f<i.wheels.length;f++){var d=d+('<div class="dwc'+("scroller"!=i.mode?" dwpm":" dwsc")+(i.showLabel?"":" dwhl")+'"><div class="dwwc dwrc"><table cellpadding="0" cellspacing="0"><tr>'),h;for(h in i.wheels[f]){var d=d+('<td><div class="dwwl dwrc">'+("scroller"!=i.mode?'<div class="dwwb dwwbp" style="height:'+c+"px;line-height:"+c+'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:'+c+"px;line-height:"+c+'px;"><span>&ndash;</span></div>':"")+'<div class="dwl">'+h+'</div><div class="dww dwrc" style="height:'+e+"px;min-width:"+i.width+'px;"><ul>'),x;for(x in i.wheels[f][h])d+='<li class="dw-v" data-val="'+x+'" style="height:'+c+"px;line-height:"+c+'px;">'+i.wheels[f][h][x]+"</li>";d+='</ul><div class="dwwo"></div></div><div class="dwwol"></div></div></td>'}d+="</tr></table></div></div>"}d+=("inline"!=i.display?'<div class="dwbc"><span class="dwbw dwb-s"><a href="#" class="dwb">'+i.setText+'</a></span><span class="dwbw dwb-c"><a href="#" class="dwb">'+i.cancelText+"</a></span></div>":'<div class="dwcc"></div>')+"</div></div></div>",y=a(d),z(),"inline"!=i.display?y.appendTo("body"):b.is("div")?b.html(y):y.insertAfter(b),K=!0,M.init(y,m),"inline"!=i.display&&(a(".dwb-s a",y).click(function(){return m.setValue(!1,!0),m.hide(),i.onSelect.call(B,m.val,m),!1}),a(".dwb-c a",y).click(function(){return m.hide(),i.onCancel.call(B,m.val,m),!1}),a("input,select").each(function(){a(this).prop("disabled")||a(this).addClass("dwtd")}),a("input,select").prop("disabled",!0),k(),a(window).bind("resize.dw",k)),y.delegate(".dwwl","DOMMouseScroll mousewheel",function(b){if(!i.readonly){b.preventDefault();var b=b.originalEvent,b=b.wheelDelta?b.wheelDelta/120:b.detail?-b.detail/3:0,c=a("ul",this),d=+c.data("pos"),d=Math.round(d-b);u(c),l(c,d,b<0?1:2)}}).delegate(".dwb, .dwwb",G,function(){a(this).addClass("dwb-a")}).delegate(".dwwb",G,function(b){if(!i.readonly){b.preventDefault(),b.stopPropagation();var c=a(this).closest(".dwwl").find("ul");func=a(this).hasClass("dwwbp")?w:E,u(c),clearInterval(g),g=setInterval(function(){func(c)},i.delay),func(c)}}).delegate(".dwwl",G,function(b){!r&&i.mode!="clickpick"&&!i.readonly&&(b.preventDefault(),r=!0,s=a("ul",this).addClass("dwa"),i.mode=="mixed"&&a(".dwwb",this).fadeOut("fast"),v=+s.data("pos"),u(s),t=o(b),A=new Date,q=t,m.scroll(s,v))}),i.onShow.call(B,y,m)},m.init=function(c){M=a.extend({defaults:{},init:n},a.scroller.themes[c.theme?c.theme:i.theme]),a.extend(i,M.defaults,c),m.settings=i,O=Math.floor(i.rows/2);var d=a.scroller.presets[i.preset];b.unbind(".dw"),d&&(d=d.call(B,m),a.extend(i,d,c),a.extend(C,d.methods)),void 0!==b.data("dwro")&&(B.readOnly=h(b.data("dwro"))),K&&m.hide(),"inline"==i.display?m.show():(p(),J&&i.showOnFocus&&(b.data("dwro",B.readOnly),B.readOnly=!0,b.bind("focus.dw",m.show)))},m.values=null,m.val=null,m.temp=null,m.init(c)}function w(b){for(var a in b)if(void 0!==z[b[a]])return!0;return!1}function o(b){return E?b.originalEvent?b.originalEvent.changedTouches[0].pageY:b.changedTouches[0].pageY:b.pageY}function h(b){return!0===b||"true"==b}function l(b,c,g,u,f){var h=b.closest(".dwwr").find("ul").index(b),c=c>d?d:c,c=c<e?e:c,b=a("li",b).eq(c);F.temp[h]=b.data("val"),F.validate(u?c==f?.1:Math.abs(.1*(c-f)):0,f,h,g)}var j={},g,n=function(){},f,e,d,F,p=(new Date).getTime(),r=!1,s=null,t,q,A,v,z=document.createElement(z).style,I=w(["perspectiveProperty","WebkitPerspective","MozPerspective","OPerspective","msPerspective"])&&"webkitPerspective"in document.documentElement.style,H=function(){var b=["Webkit","Moz","O","ms"],a;for(a in b)if(w([b[a]+"Transform"]))return"-"+b[a].toLowerCase();return""}(),E="ontouchstart"in window,G=E?"touchstart":"mousedown",L=E?"touchend":"mouseup",D={width:70,height:40,rows:3,delay:300,disabled:!1,readonly:!1,showOnFocus:!0,showLabel:!0,wheels:[],theme:"default",headerText:"{value}",display:"modal",mode:"scroller",preset:"date",setText:"Set",cancelText:"Cancel",onShow:n,onClose:n,onSelect:n,onCancel:n,onChange:n,formatResult:function(b){for(var a="",d=0;d<b.length;d++)a+=(0<d?" ":"")+b[d];return a},parseValue:function(b,a){for(var d=a.settings.wheels,b=b.split(" "),e=[],f=0,g=0;g<d.length;g++)for(var h in d[g]){if(void 0!==d[g][h][b[f]])e.push(b[f]);else for(var j in d[g][h]){e.push(j);break}f++}return e},validate:n},C={init:function(b){return void 0===b&&(b={}),this.each(function(){this.id||(p+=1,this.id="scoller"+p),j[this.id]=new k(this,b)})},enable:function(){return this.each(function(){var b=j[this.id];b&&b.enable()})},disable:function(){return this.each(function(){var b=j[this.id];b&&b.disable()})},isDisabled:function(){var b=j[this[0].id];if(b)return b.settings.disabled},option:function(b,a){return this.each(function(){var d=j[this.id];if(d){var e={};"object"==typeof b?e=b:e[b]=a,d.init(e)}})},setValue:function(b,a,d){return this.each(function(){var e=j[this.id];e&&(e.temp=b,e.setValue(!0,a,d))})},getInst:function(){return j[this[0].id]},getValue:function(){var b=j[this[0].id];if(b)return b.values},show:function(){var b=j[this[0].id];if(b)return b.show()},hide:function(){return this.each(function(){var b=j[this.id];b&&b.hide()})},destroy:function(){return this.each(function(){var b=j[this.id];b&&(b.hide(),a(this).unbind(".dw"),delete j[this.id],a(this).is("input")&&(this.readOnly=h(a(this).data("dwro"))))})}};a(document).bind(E?"touchmove":"mousemove",function(b){r&&(b.preventDefault(),q=o(b),b=v+(t-q)/f,b=b>d+1?d+1:b,b=b<e-1?e-1:b,F.scroll(s,b))}),a(document).bind(L,function(b){if(r){b.preventDefault(),s.removeClass("dwa");var c=new Date-A,b=v+(t-q)/f,b=b>d+1?d+1:b,b=b<e-1?e-1:b;300>c?(c=(q-t)/c,c=c*c/.0012,0>q-t&&(c=-c)):c=q-t,l(s,Math.round(v-c/f),0,!0,Math.round(b)),r=!1,s=null}clearInterval(g),a(".dwb-a").removeClass("dwb-a")}),a.fn.scroller=function(b){if(C[b])return C[b].apply(this,Array.prototype.slice.call(arguments,1));if("object"==typeof b||!b)return C.init.apply(this,arguments);a.error("Unknown method")},a.scroller={setDefaults:function(b){a.extend(D,b)},presets:{},themes:{}}})(jQuery),function(a){var k=new Date,w={dateFormat:"mm/dd/yy",dateOrder:"mmddy",timeWheels:"hhiiA",timeFormat:"hh:ii A",startYear:k.getFullYear()-100,endYear:k.getFullYear()+50,monthNames:"January,February,March,April,May,June,July,August,September,October,November,December".split(","),monthNamesShort:"Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec".split(","),dayNames:"Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday".split(","),dayNamesShort:"Sun,Mon,Tue,Wed,Thu,Fri,Sat".split(","),shortYearCutoff:"+10",monthText:"Month",dayText:"Day",yearText:"Year",hourText:"Hours",minuteText:"Minutes",secText:"Seconds",ampmText:"&nbsp;",stepHour:1,stepMinute:1,stepSecond:1,separator:" "},k=function(o){function h(b,a,c){return void 0!==p[a]?+b[p[a]]:void 0!==c?c:H[r[a]]?H[r[a]]():r[a](H)}function l(b,a){return Math.floor(b/a)*a}function j(b){var a=h(b,"h",0);return new Date(h(b,"y"),h(b,"m"),h(b,"d"),h(b,"ap")?a+12:a,h(b,"i",0),h(b,"s",0))}var g=a(this),n;if(g.is("input")){switch(g.attr("type")){case"text":n=g.attr("data-date-format")||"mm/dd/yyyy",w.dateOrder=n,w.dateFormat=n;break;case"datetime":n="yy-mm-ddTHH:ii:ssZ";break;case"datetime-local":n="yy-mm-ddTHH:ii:ss";break;case"month":n="yy-mm",w.dateOrder="mmyy";break;case"time":n="HH:ii:ss"}var f=g.attr("min"),g=g.attr("max");f&&(w.minDate=a.scroller.parseDate(n,f)),g&&(w.maxDate=a.scroller.parseDate(n,g))}var e=a.extend({},w,o.settings),d=0,f=[],k=[],p={},r={y:"getFullYear",m:"getMonth",d:"getDate",h:function(b){return b=b.getHours(),b=z&&12<=b?b-12:b,l(b,E)},i:function(b){return l(b.getMinutes(),G)},s:function(b){return l(b.getSeconds(),L)},ap:function(b){return v&&11<b.getHours()?1:0}},s=e.preset,t=e.dateOrder,q=e.timeWheels,A=t.match(/D/),v=q.match(/a/i),z=q.match(/h/),I="datetime"==s?e.dateFormat+e.separator+e.timeFormat:"time"==s?e.timeFormat:e.dateFormat,H=new Date,E=e.stepHour,G=e.stepMinute,L=e.stepSecond,D=e.minDate,C=e.maxDate;n=n?n:I;if(s.match(/date/i)){a.each(["y","m","d"],function(b,a){b=t.search(RegExp(a,"i")),-1<b&&k.push({o:b,v:a})}),k.sort(function(b,a){return b.o>a.o?1:-1}),a.each(k,function(b,a){p[a.v]=b});for(var g={},b=0;3>b;b++)if(b==p.y){d++,g[e.yearText]={};for(var c=D?D.getFullYear():e.startYear,Q=C?C.getFullYear():e.endYear;c<=Q;c++)g[e.yearText][c]=t.match(/yy/i)?c:(c+"").substr(2,2)}else if(b==p.m){d++,g[e.monthText]={};for(c=0;12>c;c++)g[e.monthText][c]=t.match(/MM/)?e.monthNames[c]:t.match(/M/)?e.monthNamesShort[c]:t.match(/mm/)&&9>c?"0"+(c+1):c+1}else if(b==p.d){d++,g[e.dayText]={};for(c=1;32>c;c++)g[e.dayText][c]=t.match(/dd/i)&&10>c?"0"+c:c}f.push(g)}if(s.match(/time/i)){g={};if(q.match(/h/i)){p.h=d++,g[e.hourText]={};for(c=0;c<(z?12:24);c+=E)g[e.hourText][c]=z&&0==c?12:q.match(/hh/i)&&10>c?"0"+c:c}if(q.match(/i/)){p.i=d++,g[e.minuteText]={};for(c=0;60>c;c+=G)g[e.minuteText][c]=q.match(/ii/)&&10>c?"0"+c:c}if(q.match(/s/)){p.s=d++,g[e.secText]={};for(c=0;60>c;c+=L)g[e.secText][c]=q.match(/ss/)&&10>c?"0"+c:c}v&&(p.ap=d++,d=q.match(/A/),g[e.ampmText]={0:d?"AM":"am",1:d?"PM":"pm"}),f.push(g)}return{wheels:f,headerText:function(){return a.scroller.formatDate(I,j(o.temp),e)},formatResult:function(b){return a.scroller.formatDate(n,j(b),e)},parseValue:function(b){var c=new Date,d=[];try{c=a.scroller.parseDate(n,b,e)}catch(g){}for(var f in p)d[p[f]]=c[r[f]]?c[r[f]]():r[f](c);return d},validate:function(b,c){var d=o.temp,f={m:0,d:1,h:0,i:0,s:0,ap:0},g={m:11,d:31,h:l(z?11:23,E),i:l(59,G),s:l(59,L),ap:1},j=!0,n=!0;a.each(D||C?"y,m,d,ap,h,i,s".split(","):c==p.y||c==p.m||void 0===c?["d"]:[],function(c,l){if(void 0!==p[l]){var o=f[l],i=g[l],z=31,k=h(d,l),s=a("ul",b).eq(p[l]),q,x;"d"==l&&(q=h(d,"y"),x=h(d,"m"),i=z=32-(new Date(q,x,32)).getDate(),A&&a("li",s).each(function(){var b=a(this),c=b.data("val"),d=(new Date(q,x,c)).getDay();b.html(t.replace(/[my]/gi,"").replace(/dd/,10>c?"0"+c:c).replace(/d/,c).replace(/DD/,e.dayNames[d]).replace(/D/,e.dayNamesShort[d]))})),j&&D&&(o=D[r[l]]?D[r[l]]():r[l](D)),n&&C&&(i=C[r[l]]?C[r[l]]():r[l](C));if("y"!=l){var F=a('li[data-val="'+o+'"]',s).index(),w=a('li[data-val="'+i+'"]',s).index();a("li",s).removeClass("dw-v").slice(F,w+1).addClass("dw-v"),"d"==l&&a("li",s).removeClass("dw-h").slice(z).addClass("dw-h"),k<o&&(k=o),k>i&&(k=i)}j&&(j=k==o),n&&(n=k==i);if(e.invalid&&"d"==l){var v=[];e.invalid.dates&&a.each(e.invalid.dates,function(b,a){a.getFullYear()==q&&a.getMonth()==x&&v.push(a.getDate()-1)});if(e.invalid.daysOfWeek){var I=(new Date(q,x,1)).getDay();a.each(e.invalid.daysOfWeek,function(b,a){for(var c=a-I;c<z;c+=7)c>=0&&v.push(c)})}e.invalid.daysOfMonth&&a.each(e.invalid.daysOfMonth,function(b,a){a=(a+"").split("/"),a[1]?a[0]-1==x&&v.push(a[1]-1):v.push(a[0]-1)}),a.each(v,function(b,c){a("li",s).eq(c).removeClass("dw-v")})}}})},methods:{getDate:function(b){var c=a(this).scroller("getInst");if(c)return j(b?c.temp:c.values)},setDate:function(b,c,d){return void 0==c&&(c=!1),this.each(function(){var e=a(this).scroller("getInst");if(e){for(var f in p)e.temp[p[f]]=b[r[f]]?b[r[f]]():r[f](b);e.setValue(!0,c,d)}})}}}};a.scroller.presets.date=k,a.scroller.presets.datetime=k,a.scroller.presets.time=k,a.scroller.formatDate=function(o,h,l){if(!h)return null;for(var l=a.extend({},w,l),j=function(a){for(var e=0;d+1<o.length&&o.charAt(d+1)==a;)e++,d++;return e},g=function(a,d,e){d=""+d;if(j(a))for(;d.length<e;)d="0"+d;return d},n=function(a,d,e,f){return j(a)?f[d]:e[d]},f="",e=!1,d=0;d<o.length;d++)if(e)"'"==o.charAt(d)&&!j("'")?e=!1:f+=o.charAt(d);else switch(o.charAt(d)){case"d":f+=g("d",h.getDate(),2);break;case"D":f+=n("D",h.getDay(),l.dayNamesShort,l.dayNames);break;case"o":f+=g("o",(h.getTime()-(new Date(h.getFullYear(),0,0)).getTime())/864e5,3);break;case"m":f+=g("m",h.getMonth()+1,2);break;case"M":f+=n("M",h.getMonth(),l.monthNamesShort,l.monthNames);break;case"y":f+=j("y")?h.getFullYear():(10>h.getYear()%100?"0":"")+h.getYear()%100;break;case"h":var k=h.getHours(),f=f+g("h",12<k?k-12:0==k?12:k,2);break;case"H":f+=g("H",h.getHours(),2);break;case"i":f+=g("i",h.getMinutes(),2);break;case"s":f+=g("s",h.getSeconds(),2);break;case"a":f+=11<h.getHours()?"pm":"am";break;case"A":f+=11<h.getHours()?"PM":"AM";break;case"'":j("'")?f+="'":e=!0;break;default:f+=o.charAt(d)}return f},a.scroller.parseDate=function(k,h,l){var j=new Date;if(!k||!h)return j;for(var h="object"==typeof h?h.toString():h+"",g=a.extend({},w,l),l=j.getFullYear(),n=j.getMonth()+1,f=j.getDate(),e=-1,d=j.getHours(),j=j.getMinutes(),F=0,p=-1,r=!1,s=function(a){return(a=v+1<k.length&&k.charAt(v+1)==a)&&v++,a},t=function(a){return s(a),a=h.substr(A).match(RegExp("^\\d{1,"+("@"==a?14:"!"==a?20:"y"==a?4:"o"==a?3:2)+"}")),a?(A+=a[0].length,parseInt(a[0],10)):0},q=function(a,d,e){a=s(a)?e:d;for(d=0;d<a.length;d++)if(h.substr(A,a[d].length).toLowerCase()==a[d].toLowerCase())return A+=a[d].length,d+1;return 0},A=0,v=0;v<k.length;v++)if(r)"'"==k.charAt(v)&&!s("'")?r=!1:A++;else switch(k.charAt(v)){case"d":f=t("d");break;case"D":q("D",g.dayNamesShort,g.dayNames);break;case"o":e=t("o");break;case"m":n=t("m");break;case"M":n=q("M",g.monthNamesShort,g.monthNames);break;case"y":l=t("y");break;case"H":d=t("H");break;case"h":d=t("h");break;case"i":j=t("i");break;case"s":F=t("s");break;case"a":p=q("a",["am","pm"],["am","pm"])-1;break;case"A":p=q("A",["am","pm"],["am","pm"])-1;break;case"'":s("'")?A++:r=!0;break;default:A++}100>l&&(l+=(new Date).getFullYear()-(new Date).getFullYear()%100+(l<=g.shortYearCutoff?0:-100));if(-1<e){n=1,f=e;do{g=32-(new Date(l,n-1,32)).getDate();if(f<=g)break;n++,f-=g}while(1)}d=new Date(l,n-1,f,-1==p?d:p&&12>d?d+12:!p&&12==d?0:d,j,F);if(d.getFullYear()!=l||d.getMonth()+1!=n||d.getDate()!=f)throw"Invalid date";return d}}(jQuery),function(a){var k={inputClass:""};a.scroller.presets.select=function(w){var o=a.extend({},k,w.settings),h=a(this),l=this.id+"_dummy";a('label[for="'+this.id+'"]').attr("for",l);var j=a('label[for="'+l+'"]'),j=j.length?j.text():h.attr("name"),g=[],n=[{}];n[0][j]={};var f=n[0][j];a("option",h).each(function(){var d=a(this).attr("value");f[d]=a(this).text(),a(this).prop("disabled")&&g.push(d)}),a("#"+l).remove();var e=a('<input type="text" id="'+l+'" value="'+f[h.val()]+'" class="'+o.inputClass+'" readonly />').insertBefore(h);return o.showOnFocus&&e.focus(function(){w.show()}),h.hide().closest(".ui-field-contain").trigger("create"),{width:200,wheels:n,headerText:!1,formatResult:function(a){return f[a[0]]},parseValue:function(){return[h.val()]},validate:function(d){a.each(g,function(e,f){a('li[data-val="'+f+'"]',d).removeClass("dw-v")})},onSelect:function(a,f){e.val(a),h.val(f.values[0]).trigger("change")},onChange:function(a,f){"inline"==o.display&&(e.val(a),h.val(f.temp[0]).trigger("change"))},onClose:function(){e.blur()}}}}(jQuery),function(a){a.scroller.themes.android={defaults:{dateOrder:"Mddyy",mode:"clickpick",height:50}}}(jQuery),function(a){var k={defaults:{dateOrder:"Mddyy",mode:"mixed",rows:5,width:70,showLabel:!1}};a.scroller.themes["android-ics"]=k,a.scroller.themes["android-ics light"]=k}(jQuery),function(a){a.scroller.themes.ios={defaults:{dateOrder:"MMdyy",rows:5,height:30,width:55,headerText:!1,showLabel:!1}}}(jQuery),function(a){a.scroller.themes.jqm={defaults:{jqmBody:"c",jqmHeader:"b",jqmWheel:"d",jqmClickPick:"c",jqmSet:"b",jqmCancel:"c"},init:function(k,w){var o=w.settings;a(".dw",k).removeClass("dwbg").addClass("ui-overlay-shadow ui-corner-all ui-body-a"),a(".dwb-s a",k).attr("data-role","button").attr("data-theme",o.jqmSet),a(".dwb-c a",k).attr("data-role","button").attr("data-theme",o.jqmCancel),a(".dwwb",k).attr("data-role","button").attr("data-theme",o.jqmClickPick),a(".dwv",k).addClass("ui-header ui-bar-"+o.jqmHeader),a(".dwwr",k).addClass("ui-body-"+o.jqmBody),a(".dwpm .dww",k).addClass("ui-body-"+o.jqmWheel),"inline"!=o.display&&a(".dw",k).addClass("pop in"),k.trigger("create"),a(".dwo",k).click(function(){w.hide()})}}}(jQuery);function sscGetDateFormat(fmt){for(var padding=0,tmp=0,res="",len=fmt.length,ii=0;ii<len;ii++)if(fmt[ii]>31)res+=fmtstrings[fmt[ii]-32];else switch(fmt[ii]){case 2:res+="m";break;case 3:res+="mm";break;case 4:res+="M";break;case 5:res+="MM";break;case 6:res+="d";break;case 7:res+="dd";break;case 8:res+="D";break;case 9:res+="DD";break;case 10:res+="yy";break;case 11:res+="yy"}return res}
...
;
			$(function(){
                ...
                eachInvalidField : function() {
                    if(bPopup===false) $('#popupDialog').popup('open');bPopup=true;
                    if($(this).is('input:radio')) $(this).closest('div.ui-controlgroup-controls').addClass('error');
...
recalc_onclick('obj');
$('input[type=text]').each(function(i) { var str = this.onchange == null ?  '' :this.onchange.toString(); var varName = str.match(/fmtdate\d+/); if(varName && varName.length > 0)     {     var dtFmt=sscGetDateFormat(window[varName[0]]);      $(this).attr({ 'data-date-format':dtFmt});    } });
$('input[type=text][data-date-format]').scroller();
        ...
        function closepopup(){$('#popupDialog').popup('close');bPopup=false;}
var eeisus=0;var eetrue="TRUE";var eefalse="FALSE";var eedec=".";var eeth=" ";var eedecreg=new RegExp("\\.","g");var eethreg=new RegExp(" ","g");var eecurrencyreg=new RegExp("R","g");var eepercentreg=new RegExp("%","g"); var fmtdaynamesshort=new Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat"); var fmtdaynameslong=new Array("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"); var fmtmonthnamesshort=new Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"); var fmtmonthnameslong=new Array("January","February","March","April","May","June","July","August","September","October","November","December"); var fmtstrings=new Array(":"," ","/"); var fmtdate1=[14,32,17,32,19,33,21]; var fmtdate2=[11,34,3,34,7];
...
var row3xB9B31=new Array(23);for(var jj=0;jj<23;jj++){row3xB9B31[jj]=0};var row3xC9C31=new Array(23);for(var jj=0;jj<23;jj++){row3xC9C31[jj]=""};var row3xD9D31=new Array(23);for(var jj=0;jj<23;jj++){row3xD9D31[jj]=0};var row3xE9E31=new Array(23);for(var jj=0;jj<23;jj++){row3xE9E31[jj]=0};function calc(data){var c1C16=data['extra2'];var c1C15=data['extra1'];var c1C14=data['altitude'];var c1C12=data['yield'];var c1C8=data['trapnr'];var c1C6=data['trapdate'];var c1C7=(now());row3xB9B31[0]=(1);row3xC9C31[0]=("African Pride");row3xD9D31[0]=(33.91403);row3xE9E31[0]=(20.4288);row3xB9B31[1]=(2);row3xC9C31[1]=("Sapphire");row3xD9D31[1]=(33.9133);row3xE9E31[1]=(20.42773);row3xB9B31[2]=(3);row3xC9C31[2]=("Nuweland");row3xD9D31[2]=(33.91288);row3xE9E31[2]=(20.42877);row3xB9B31[3]=(4);row3xC9C31[3]=("Nuwe S & L");row3xD9D31[3]=(33.91273);row3xE9E31[3]=(20.43004);row3xB9B31[4]=(5);row3xC9C31[4]=("Hange");row3xD9D31[4]=(33.91247);row3xE9E31[4]=(20.42759);row3xB9B31[5]=(6);row3xC9C31[5]=("S & L");row3xD9D31[5]=(33.91355);row3xE9E31[5]=(20.43097);row3xB9B31[6]=(7);row3xC9C31[6]=("Kakamas");row3xD9D31[6]=(33.91236);row3xE9E31[6]=(20.43154);row3xB9B31[7]=(8);row3xC9C31[7]=("Kareebos");row3xD9D31[7]=(33.91347);
 row3xE9E31[7]=(20.43283);row3xB9B31[8]=(9);row3xC9C31[8]=("Sandvliet");row3xD9D31[8]=(33.91312);row3xE9E31[8]=(20.43354);row3xB9B31[9]=(10);row3xC9C31[9]=("Jong Keisie");row3xD9D31[9]=(33.9146);row3xE9E31[9]=(20.43577);row3xB9B31[10]=(11);row3xC9C31[10]=("Ou Keisie");row3xD9D31[10]=(33.91369);row3xE9E31[10]=(20.43794);row3xB9B31[11]=(12);row3xC9C31[11]=("Imperial");row3xD9D31[11]=(33.91661);row3xE9E31[11]=(20.43063);row3xB9B31[12]=(13);row3xC9C31[12]=("Skool Onder");row3xD9D31[12]=(33.91821);row3xE9E31[12]=(20.43015);row3xB9B31[13]=(14);row3xC9C31[13]=("Skool Middel");row3xD9D31[13]=(33.91854);row3xE9E31[13]=(20.43074);row3xB9B31[14]=(15);row3xC9C31[14]=("WW Keisie");row3xD9D31[14]=(33.92336);row3xE9E31[14]=(20.43194);row3xB9B31[15]=(16);row3xC9C31[15]=("Fantasy");row3xD9D31[15]=(33.92219);row3xE9E31[15]=(20.43115);row3xB9B31[16]=(17);row3xC9C31[16]=("Supreme");row3xD9D31[16]=(33.93066);row3xE9E31[16]=(20.43647);row3xB9B31[17]=(18);row3xC9C31[17]=("Cascade");row3xD9D31[17]=(33.93131);row3xE9E31[17]=(20.43554);row3xB9B31[18]=(19);row3xC9C31[18]=("Bulida");row3xD9D31[18]=(33.90584);row3xE9E31[18]=(20.40483);row3xB9B31[19]=(20);
 row3xC9C31[19]=("Malherbe");row3xD9D31[19]=(33.90537);row3xE9E31[19]=(20.4032);row3xB9B31[20]=(21);row3xC9C31[20]=("Southern Bell 01");row3xD9D31[20]=(33.90574);row3xE9E31[20]=(20.40338);row3xB9B31[21]=(22);row3xC9C31[21]=("Colombard");row3xD9D31[21]=(33.90693);row3xE9E31[21]=(20.40362);row3xB9B31[22]=(23);row3xC9C31[22]=("Southern Bell 02");row3xD9D31[22]=(33.90682);row3xE9E31[22]=(20.40512);var c1C9=(lookup3vv((c1C8),row3xB9B31,0,22,row3xC9C31,0,22));var c1C10=(lookup3vv((c1C8),row3xB9B31,0,22,row3xD9D31,0,22));var c1C11=(lookup3vv((c1C8),row3xB9B31,0,22,row3xE9E31,0,22));data['longitude']=c1C11;data['latitude']=c1C10;data['name']=c1C9;data['time']=c1C7;};
function postcode() {
 document.getElementById('trapdate').value=eedatefmt(fmtdate2,41934);
};
var co = new Object;
function recalc_onclick(ctl) {
  if (true) {
co['trapdate']=eeparsedate_keep_all(document.getElementById('trapdate').value);co['trapnr']=eeparseFloat(document.getElementById('trapnr').value);co['yield']=document.getElementById('yield').value;co['altitude']=document.getElementById('altitude').value;co['extra1']=document.getElementById('extra1').value;co['extra2']=document.getElementById('extra2').value;calc(co);document.getElementById('time').value=eedatefmt(fmtdate1,co['time']);document.getElementById('name').value=eeisnumber(co['name'])?eedisplayFloat(co['name']):co['name'];document.getElementById('latitude').value=eedisplayFloat(co['latitude']);document.getElementById('longitude').value=eedisplayFloat(co['longitude']);
};};
function reset_onclick(){document.forms[0].reset();if(typeof recalc_onclick == 'function'){recalc_onclick('');}}
